...
<div class="fixed z-10 inset-0 invisible overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true" id="edit{{ $user->name }}">
                                 ...
                                 <div class="mt-4">
                                     <div>
                                         <label class="block" for="Name">Name<label>
                                                 <input type="text" name="name" values="{{ $user->name }}"
                                     ...
                                     <div class="mt-4">
                                         <label class="block" for="email">Email<label>
                                                 <input type="text" name="email" values="{{ $user->email }}"
                                                     class="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600">
                                                 @if ($errors->has('email'))
...
<script type="text/javascript">
    $(document).ready(function () {
        $('.modalOpen').on('click', function(e){
            $('#edit{{ $user->name }}').removeClass('invisible');
        });
        $('.modalClose').on('click', function(e){
            $('#edit{{ $user->name }}').addClass('invisible');
        });
    });
</script>
